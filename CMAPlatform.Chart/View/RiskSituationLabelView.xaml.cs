...
namespace CMAPlatform.Chart.View
{
    ...
    ///     风险态势标牌视图(落区标牌)
    /// </summary>
    public partial class RiskSituationLabelView : ChartViewBase
    {
        ...
        public override void Dispose()
        {
            if (Controllers != null)
            {
                ...
                {
                    var riskSituationController = controller as GIS3DRiskSituationLabelController;
                    m_RiskSituationController = riskSituationController;
                    if (riskSituationController != null)
                    {
                        riskSituationController.AddLabelEvent -= riskSituationController_AddLabelEvent;
                        riskSituationController.DeleteLabelEvent -= riskSituationController_DeleteLabelEvent;
                    }
        ...
        ///     风险态势标牌控制器(落区标牌)
        /// </summary>
        private GIS3DRiskSituationLabelController m_RiskSituationController;
        ...
        public RiskSituationLabelView(ChartViewBaseModel model)
            : base(model)
        {
            InitializeComponent();
            ...
            else
            {
                ...
                contentControl.Content = m_GIS3DGlobe;
        ...
        private void GIS3DGlobe_OnEngineInitialized(object sender, EventArgs e)
        {
            ...
            EngineContainer.SurfaceView.Globe3DControler.RenderTarget.MouseClick += RenderTarget_MouseClick;
            foreach (GIS3DControllerBase controller in Controllers)
            {
        ...
        private void RenderTarget_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                {
                    SelectFromScreen(e.X, e.Y);
                }
            }
            catch (Exception ex)
            {
                throw new Exception("SurfaceView----m_Globe3DControler_MouseClick----" + ex.Message);
            }
        }

        private void SelectFromScreen(int x, int y)
        {
            var buildingId = "";

            var selectedEntity = EngineContainer.SurfaceView.Globe3DControler.World.World.SelectFromScreen(x, y,
                ref buildingId);
            if (selectedEntity != null)
            {
                if (selectedEntity.Name.StartsWith(m_RiskSituationController.DVM.Name + "clickentity_"))
                {
                    var key = selectedEntity.Name.Replace(m_RiskSituationController.DVM.Name + "clickentity_", "");
                    //这里拿到标牌的key
                    m_MessageAggregator.GetMessage<CMAClickWarningBillboardMessage>().Publish(key);
                }
            }
        }
        ...
        {
            foreach (var child in grdHidden.Children)
            {
                var control = child as RiskSituationLabelControl;
                if (control != null)
                {
                    control.Loaded -= control_Loaded;
                }
            }
            grdHidden.Children.Clear();
        }
        ...
        private void riskSituationController_AddLabelEvent(List<RiskSituationLabelControl> labelControls)
        {
            foreach (var control in labelControls)
            {
                grdHidden.Children.Add(control);
                control.Loaded += control_Loaded;
            }
        }
        ...
        {
            if (m_RiskSituationController == null) return;
            m_RiskSituationController.AddToMap(sender as RiskSituationLabelControl);
        }
